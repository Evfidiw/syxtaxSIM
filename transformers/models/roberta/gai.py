...
class RobertaEncoder(nn.Module):
    ...
    def forward(
        ...
        if output_hidden_states:
            ...
            # layer_module(span_hidden_states, span_mask)
            for i, layer_module in enumerate(self.span_layer):
                if output_hidden_states:
                    span_hidden_states = span_hidden_states + (hidden_states,)
...
class RobertaModel(RobertaPreTrainedModel):
    ...
    def forward(
        ...
        if span_mask is not None:
            extended_span_mask: torch.Tensor = self.get_extended_attention_mask(span_mask, input_shape,
                                                                                     device)
            if self.config.is_decoder and encoder_hidden_states is not None:
                encoder_batch_size, encoder_sequence_length, _ = encoder_hidden_states.size()
                encoder_hidden_shape = (encoder_batch_size, encoder_sequence_length)
                encoder_attention_mask = torch.ones(encoder_hidden_shape, device=device)
                encoder_extended_span_mask = self.invert_attention_mask(encoder_attention_mask)
        ...
        if span_mask is not None:
            encoder_outputs = self.encoder(
                embedding_output,
                span_mask=extended_span_mask,
                attention_mask=extended_attention_mask,
                head_mask=head_mask,
                encoder_hidden_states=encoder_hidden_states,
                encoder_attention_mask=encoder_extended_span_mask,
